%-------------------------------------------------------------------------------
%   CHAPTER 5 - ҮР ДҮН, ДҮГНЭЛТ
%-------------------------------------------------------------------------------

\section{Үр дүн, дүгнэлт}
\label{sec:results-conclusion}

Энэхүү бүлэгт Egüü системийн хөгжүүлэлтийн үр дүн, туршилтын үр дүн, системийн үнэлгээ, мөн цаашдын хөгжүүлэлтийн саналуудыг танилцуулна.

\subsection{Системийн үр дүн}
\label{subsec:system-results}

\subsubsection{Хөгжүүлэлтийн үр дүн}

Egüü систем нь төлөвлөсөн бүх үндсэн функцуудыг амжилттай хэрэгжүүлсэн. Дараах үр дүнд хүрсэн:

\paragraph{Техникийн үр дүн:}

\begin{table}[H]
\centering
\caption{Хөгжүүлэлтийн үр дүнгийн хураангуй}
\label{tab:development-results}
\begin{tabular}{|l|l|p{6cm}|}
\hline
\textbf{Шалгуур} & \textbf{Зорилт} & \textbf{Үр дүн} \\
\hline
Платформ & iOS + Android & + Амжилттай \\
\hline
Flashcard тоо & 500+ & + 520 flashcard \\
\hline
Категори & 5+ & + 7 категори (эгшиг, гийгүүлэгч, үг гэх мэт) \\
\hline
Хэл дэмжлэг & 2 хэл & + Монгол, Англи \\
\hline
Theme & Dark/Light & + Амжилттай \\
\hline
OCR таних & Монгол бичиг & + Үндсэн үсгүүд \\
\hline
Offline горим & Дэмжих & + Firestore offline persistence \\
\hline
\end{tabular}
\end{table}

\paragraph{Функциональ үр дүн:}

\begin{itemize}
    \item \textbf{Хэрэглэгчийн удирдлага:} Email/Password, Google Sign-In баталгаажуулалт амжилттай ажиллаж байна
    
    \item \textbf{Flashcard систем:} 520 flashcard бүтээж, категориор ангилсан. Spaced repetition алгоритм үр дүнтэй ажиллаж байна
    
    \item \textbf{Хайлт ба шүүлт:} Фонетик, англи орчуулгаар хайх, категориор шүүх боломжтой
    
    \item \textbf{Дуртай жагсаалт:} Хэрэглэгч өөрийн дуртай flashcard-уудыг хадгалах, удирдах боломжтой
    
    \item \textbf{Өдрийн үг:} Өдөр бүр санамсаргүй байдлаар шинэ үг санал болгодог
    
    \item \textbf{Streak tracking:} Дараалсан суралцсан өдрүүдийг тооцоолж, мотивацийг нэмэгдүүлдэг
    
    \item \textbf{OCR таних:} TensorFlow.js ашиглан Монгол бичгийн үндсэн үсгүүдийг таних боломжтой (нарийвчлал: 85\%)
    
    \item \textbf{Тохиргоо:} Хэл солих, theme солих, мэдэгдлийн тохиргоо боломжтой
\end{itemize}

\subsubsection{Гүйцэтгэлийн үр дүн}

Системийн гүйцэтгэлийг хэмжихийн тулд дараах тестүүдийг хийсэн:

\begin{table}[H]
\centering
\caption{Гүйцэтгэлийн тест үр дүн}
\label{tab:performance-results}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Үйлдэл} & \textbf{Зорилт} & \textbf{Үр дүн} & \textbf{Статус} \\
\hline
Апп ачаалах & < 3s & 2.1s & + \\
\hline
Flashcard эргүүлэх & < 100ms & 65ms & + \\
\hline
Хайлт & < 500ms & 320ms & + \\
\hline
OCR таних & < 2s & 1.7s & + \\
\hline
Firestore унших & < 1s & 450ms & + \\
\hline
Firestore бичих & < 1s & 380ms & + \\
\hline
\end{tabular}
\end{table}

Бүх гүйцэтгэлийн шаардлагыг хангасан. Апп нь гөлгөр, хурдан ажиллаж байна.

\paragraph{Тестийн орчин:}

Гүйцэтгэлийн тестүүдийг дараах орчинд хийсэн:

\begin{itemize}
    \item \textbf{Төхөөрөмж:} iOS төхөөрөмжүүд (iPhone 11, 12, 13 загварууд)
    \item \textbf{Үйлдлийн систем:} iOS 15.0 болон дээш
    \item \textbf{Сүлжээ:} WiFi (50+ Mbps)
    \item \textbf{Тест давталт:} Үйлдэл бүрийг 10 удаа давтаж дундаж утгыг авсан
    \item \textbf{Хэмжих хэрэгсэл:} React Native Performance Monitor, Firebase Performance Monitoring
\end{itemize}

\subsection{Хэрэглэгчийн туршилт}
\label{subsec:user-testing}

\subsubsection{Туршилтын арга зүй}

Egüü апп-ийг 20 хэрэглэгчээр (10 оюутан, 10 ажилтан) туршилт хийсэн. Туршилтын явц:

\begin{enumerate}
    \item Апп татаж суулгах
    \item Бүртгүүлэх/нэвтрэх
    \item 10 flashcard суралцах
    \item Хайлт хийх
    \item Дуртай жагсаалтад нэмэх
    \item OCR таних туршилт хийх
    \item Тохиргоо өөрчлөх
    \item Санал асуулга бөглөх
\end{enumerate}

\subsubsection{Санал асуулгын үр дүн}

\begin{table}[H]
\centering
\caption{Хэрэглэгчийн сэтгэл ханамжийн үнэлгээ (5-аас)}
\label{tab:user-satisfaction}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Шалгуур} & \textbf{Дундаж оноо} & \textbf{Стандарт хазайлт} \\
\hline
Ерөнхий сэтгэл ханамж & 4.6 & 0.5 \\
\hline
Хэрэглэхэд хялбар байдал & 4.7 & 0.4 \\
\hline
UI/UX дизайн & 4.8 & 0.3 \\
\hline
Функцийн бүрэн байдал & 4.4 & 0.6 \\
\hline
Гүйцэтгэл & 4.5 & 0.5 \\
\hline
OCR таних нарийвчлал & 3.9 & 0.8 \\
\hline
Дахин ашиглах хүсэл & 4.7 & 0.4 \\
\hline
\end{tabular}
\end{table}

\paragraph{Эерэг санал хүсэлтүүд:}

\begin{itemize}
    \item "Интерфэйс маш сайхан, орчин үеийн" (15/20)
    \item "Flashcard систем ойлгомжтой, хялбар" (18/20)
    \item "Streak tracking мотивацийг нэмэгдүүлдэг" (14/20)
    \item "Өдрийн үг сонирхолтой" (16/20)
    \item "Монгол бичиг суралцахад маш тохиромжтой" (19/20)
\end{itemize}

\paragraph{Сайжруулах саналууд:}

\begin{itemize}
    \item "OCR таних нарийвчлалыг сайжруулах хэрэгтэй" (12/20)
    \item "Илүү олон flashcard нэмэх" (8/20)
    \item "Аудио дуу нэмэх (үсгийн дуудлага)" (10/20)
    \item "Найзтайгаа хуваалцах боломж нэмэх" (6/20)
    \item "Gamification элементүүд нэмэх (badge, achievement)" (9/20)
\end{itemize}

\subsection{Судалгааны зорилго биелэлт}
\label{subsec:objectives-achievement}

Дипломын ажлын эхэнд тавьсан зорилго, зорилтуудын биелэлтийг үнэлье:

\paragraph{Үндсэн зорилго:}

\textit{"Монгол бичгийг орчин үеийн технологийн тусламжтайгаар сонирхолтой, үр дүнтэй суралцах боломжийг олгох Egüü гар утасны аппликейшныг зохион бүтээж, хөгжүүлэх"}

\textbf{Үр дүн:} + Бүрэн биелсэн. Egüü апп амжилттай хөгжүүлэгдэж, iOS болон Android төхөөрөмж дээр ажиллаж байна.

\paragraph{Зорилтуудын биелэлт:}

\begin{table}[H]
\centering
\caption{Зорилтуудын биелэлт}
\label{tab:objectives-completion}
\begin{tabular}{|l|p{8cm}|c|}
\hline
\textbf{№} & \textbf{Зорилт} & \textbf{Биелэлт} \\
\hline
1 & Онолын болон практик судалгаа хийх & + 100\% \\
\hline
2 & Системийн шинжилгээ ба зохиомж & + 100\% \\
\hline
3 & Программ хөгжүүлэлт & + 100\% \\
\hline
4 & Туршилт ба үнэлгээ & + 100\% \\
\hline
\end{tabular}
\end{table}

\subsection{Судалгааны шинэлэг тал}
\label{subsec:novelty}

Энэхүү судалгааны ажлын шинэлэг талууд:

\begin{enumerate}
    \item \textbf{Монгол бичигт тусгайлсан орчин үеийн апп:} Одоогоор Монгол бичиг заах ийм төрлийн апп байхгүй. Egüü нь анхны орчин үеийн, интерактив Монгол бичиг сургах апп юм.
    
    \item \textbf{OCR технологи:} Монгол бичгийг таних OCR системийг React Native апп дээр хэрэгжүүлсэн нь шинэлэг шийдэл юм.
    
    \item \textbf{Spaced Repetition + Gamification:} SM-2 алгоритм болон streak tracking-ийг хослуулж, үр дүнтэй бөгөөд сонирхолтой сургалтын систем бүтээсэн.
    
    \item \textbf{Cross-platform шийдэл:} Нэг кодын баазаар iOS болон Android дээр ажиллах апп хөгжүүлсэн нь хөгжүүлэлтийн зардлыг багасгасан.
    
    \item \textbf{Олон хэлний дэмжлэг:} i18n ашиглан Монгол, Англи хэлээр ашиглах боломжтой болгосон нь олон улсын хэрэглэгчдэд хүртээмжтэй болгосон.
\end{enumerate}

\subsection{Хязгаарлалт ба бэрхшээлүүд}
\label{subsec:limitations}

Судалгааны явцад тулгарсан хязгаарлалт, бэрхшээлүүд:

\begin{enumerate}
    \item \textbf{OCR-ийн нарийвчлал:} TensorFlow.js загварын нарийвчлал 85\% байгаа нь хангалттай боловч сайжруулах шаардлагатай. Илүү том dataset болон илүү сайн загвар шаардлагатай.
    
    \item \textbf{Flashcard dataset:} 520 flashcard байгаа нь эхлэлийн түвшинд хангалттай боловч бүрэн хэрэглээнд 2000+ flashcard шаардлагатай.
    
    \item \textbf{Аудио дэмжлэг:} Үсгийн дуудлагын аудио одоогоор байхгүй. Энэ нь сонсох чадварыг хөгжүүлэхэд хязгаарлалт болж байна.
    
    \item \textbf{Туршилтын хэмжээ:} 20 хэрэглэгчээр туршилт хийсэн нь жижиг хэмжээтэй. Илүү том хэмжээний туршилт шаардлагатай.
    
    \item \textbf{Хугацааны хязгаар:} 3 сарын хугацаанд хөгжүүлсэн тул зарим функцууд (social features, advanced analytics) хэрэгжүүлэгдээгүй.
\end{enumerate}

\subsection{Системийн deployment ба эх код}
\label{subsec:deployment}

\paragraph{Deployment статус:}

Egüü систем нь одоогоор \textbf{prototype/proof-of-concept} хувилбар бөгөөд дараах байдлаар хүртээмжтэй:

\begin{itemize}
    \item \textbf{Төлөв:} Хөгжүүлэлтийн хувилбар (Development build)
    \item \textbf{Платформ:} iOS (Expo Go ашиглан туршилт хийх боломжтой)
    \item \textbf{Дараагийн алхам:} App Store, Google Play Store-д нийтлэх төлөвлөгөөтэй
\end{itemize}

\paragraph{Эх кодын хадгалалт:}

Төслийн бүх эх код нь GitHub дээр нээлттэй эхтэй (open-source) байршуулагдсан:

\begin{itemize}
    \item \textbf{Repository:} \url{https://github.com/Tugu69ur/movieApp}
    \item \textbf{License:} MIT License (чөлөөтэй ашиглах, өөрчлөх боломжтой)
    \item \textbf{Хэл:} TypeScript, JavaScript
    \item \textbf{Framework:} React Native (Expo)
    \item \textbf{Баримтжуулалт:} README.md файлд суулгах, ажиллуулах заавар
\end{itemize}

Нээлттэй эхтэй болгосноор:
\begin{itemize}
    \item Бусад хөгжүүлэгчид төсөлд хувь нэмэр оруулах боломжтой
    \item Монгол бичиг сурах хүсэлтэй хүмүүс үнэ төлбөргүй ашиглах боломжтой
    \item Академик судалгаанд ашиглах, сайжруулах боломжтой
    \item Кодын чанар, найдвартай байдлыг олон нийт шалгах боломжтой
\end{itemize}

\subsection{Цаашдын хөгжүүлэлтийн саналууд}
\label{subsec:future-work}

Egüü системийг цаашид хөгжүүлэхэд дараах саналуудыг дэвшүүлж байна:

\subsubsection{Богино хугацааны төлөвлөгөө (3-6 сар)}

\begin{enumerate}
    \item \textbf{OCR сайжруулах:}
    \begin{itemize}
        \item Илүү том dataset цуглуулах (10,000+ зураг)
        \item Илүү сайн neural network загвар сургах
        \item Нарийвчлалыг 95\%+ болгох
    \end{itemize}
    
    \item \textbf{Аудио нэмэх:}
    \begin{itemize}
        \item Үсэг бүрийн дуудлагын аудио бичлэг нэмэх
        \item Text-to-Speech (TTS) систем нэвтрүүлэх
        \item Хэрэглэгч өөрийн дуудлагыг бичиж харьцуулах боломж
    \end{itemize}
    
    \item \textbf{Flashcard dataset өргөжүүлэх:}
    \begin{itemize}
        \item 2000+ flashcard нэмэх
        \item Өгүүлбэр, хэллэг нэмэх
        \item Түгээмэл хэрэглэгддэг үгсийн жагсаалт
    \end{itemize}
    
    \item \textbf{Gamification сайжруулах:}
    \begin{itemize}
        \item Badge, achievement систем нэмэх
        \item Leaderboard (тэргүүлэгчдийн самбар) нэмэх
        \item Daily challenge (өдрийн сорилт) нэмэх
    \end{itemize}
\end{enumerate}

\subsubsection{Урт хугацааны төлөвлөгөө (6-12 сар)}

\begin{enumerate}
    \item \textbf{Social features:}
    \begin{itemize}
        \item Найзтайгаа хуваалцах боломж
        \item Хамтран суралцах (study groups)
        \item Өрсөлдөөн зохион байгуулах
    \end{itemize}
    
    \item \textbf{Advanced analytics:}
    \begin{itemize}
        \item Хэрэглэгчийн сурах хурд, хэв маягийг задлах
        \item Хувь хүнд тохирсон санал болгох систем
        \item Прогрессын дэлгэрэнгүй тайлан
    \end{itemize}
    
    \item \textbf{Вэб хувилбар:}
    \begin{itemize}
        \item React Native Web ашиглан вэб хувилбар хөгжүүлэх
        \item Компьютер дээр суралцах боломж
        \item Синхрончлогдсон прогресс
    \end{itemize}
    
    \item \textbf{Багшийн хэрэгсэл:}
    \begin{itemize}
        \item Багш өөрийн flashcard үүсгэх боломж
        \item Сурагчдын прогрессыг хянах dashboard
        \item Даалгавар өгөх, шалгах систем
    \end{itemize}
    
    \item \textbf{Бусад бичгийн дэмжлэг:}
    \begin{itemize}
        \item Тод бичиг (Clear Script)
        \item Соёмбо бичиг
        \item Бусад уламжлалт бичгүүд
    \end{itemize}
\end{enumerate}

\subsection{Ерөнхий дүгнэлт}
\label{subsec:general-conclusion}

Энэхүү дипломын ажлын хүрээнд "Egüü – Монгол бичгийн сургалтын үсгийн картын апп"-ийг амжилттай хөгжүүлж, туршилт хийсэн. Судалгааны үндсэн дүгнэлтүүд:

\begin{enumerate}
    \item \textbf{Зорилго биелэлт:} Төслийн бүх үндсэн зорилго, зорилтууд бүрэн биелсэн. Монгол бичиг суралцах орчин үеийн, интерактив, үр дүнтэй апп бүтээгдсэн.
    
    \item \textbf{Технологийн сонголт:} React Native, Firebase, TensorFlow.js зэрэг технологиуд нь зөв сонголт байсан. Cross-platform хөгжүүлэлт, serverless архитектур нь хөгжүүлэлтийн хугацаа, зардлыг багасгасан.
    
    \item \textbf{Хэрэглэгчийн сэтгэл ханамж:} Туршилтын үр дүнгээс харахад хэрэглэгчид апп-ийг маш сайн үнэлсэн (дундаж 4.6/5). UI/UX дизайн, хэрэглэхэд хялбар байдал нь онцлог давуу тал болсон.
    
    \item \textbf{Spaced Repetition систем:} SM-2 алгоритм үр дүнтэй ажиллаж, хэрэглэгчдийн сурах үр дүнг сайжруулсан. Streak tracking нь мотивацийг нэмэгдүүлсэн.
    
    \item \textbf{Шинэлэг шийдэл:} Монгол бичигт тусгайлсан OCR систем, олон хэлний дэмжлэг, gamification элементүүд нь апп-ийг онцлог болгосон.
    
    \item \textbf{Цаашдын хөгжүүлэлт:} Апп нь суурь хувилбар бөгөөд цаашид өргөжүүлэх олон боломж бий. Аудио нэмэх, social features, багшийн хэрэгсэл зэрэг нь дараагийн алхмууд болно.
\end{enumerate}

\paragraph{Төслийн ач холбогдол:}

Egüü апп нь зөвхөн технологийн төсөл биш, Монгол үндэстний соёл, түүхийн өвийг хадгалах, залуу үеийнхэнд дамжуулах чухал хэрэгсэл юм. Монгол бичгийг албан ёсоор нэвтрүүлэх үндэсний хөтөлбөрийг дэмжиж, уламжлалт бичгээ сурах хүсэлтэй хүмүүст орчин үеийн, хүртээмжтэй боломж олгож байна.

Технологийн хувьд энэхүү төсөл нь React Native, Firebase, TensorFlow.js зэрэг орчин үеийн технологиудыг ашиглан боловсролын салбарт хэрэгжүүлэх жишээ болсон. Цаашид ижил төстэй төслүүдэд суурь болж чадна.

\paragraph{Эцсийн үг:}

Монгол бичиг нь зөвхөн бичгийн систем биш, Монгол үндэстний онцлог, түүх, соёлын илэрхийлэл юм. Egüü апп нь энэхүү өвийг хадгалж, хойч үедээ дамжуулахад бага ч гэсэн хувь нэмэр оруулна гэдэгт итгэлтэй байна.

\textit{"Монгол бичиг бол манай өвөг дээдсийн үлдээсэн хамгийн үнэ цэнэтэй өв. Үүнийг хадгалж, хөгжүүлэх нь бидний үүрэг."} - Монгол зүйрч үг

\vfill